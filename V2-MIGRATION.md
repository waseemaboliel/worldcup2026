...
| `src/features/standings.js` | 447 | `computeStandings()` (groups matches by IdGroup, accumulates results + live scores + ESPN card data), `fifaGroupSort()` (FIFA tiebreaker chain: H2H pts/GD/GF → overall GD/GF → fair play), `computeBestThirds()` (ranks all 12 third-placed teams by overall stats), `computeClinched()` (H2H-aware — checks if team mathematically guaranteed top 2, suppressed during live matches), `renderStandings()` (sub-tabs Groups/Best Thirds, sticky below nav, 15s auto-refresh), `renderGroupStandings()` (group tables with qualify/qualify-third/clinched classes), `renderBestThirdsTable()` (full 12-team ranking, top 8 qualified, bottom 4 eliminated), `stopLiveStandingsPoller()`. Private: `isGroupStage()`, `bestThirdsSort()`, `resolveWithH2H()` |
